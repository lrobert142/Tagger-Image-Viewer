...
class Main extends Component {
    ...
    handleSearchChange(search) {
        ...
        let filteredImages = this.state.images.filter((entry) => {
            return entry.title.toLowerCase().includes(lowercaseSearch) || entry.tags.toString().toLowerCase().includes(lowercaseSearch);
        });
    ...
    render() {
        ...
        return (
            <div className="Main">
                ...
                <ImageModal modalActive={this.state.modalActive} title={activeImage.title} tags={activeImage.tags} image={activeImage.src} handleClose={this.toggleModal}/> {thumbnails}
            </div>
        );
    }
}
